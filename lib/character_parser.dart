...
SrcPosition parseMax(src, {int start:0}){
  ...
  while (state.roundDepth >= 0 && state.curlyDepth >= 0 && state.squareDepth >= 0) {
    if (index >= src.length) {
      throw new ParseError('The end of the string was reached with no closing bracket found.');
    }
    parseChar(src[index++], state);
  }
  var end = index - 1;
...
ParserState parseChar(character, [ParserState state]){
  if (character.length != 1) throw new ParseError('Character must be a string of length 1');
  if (state == null) state = defaultState();
